...
namespace Assets.FantasyHeroes.Scripts
{
    ...
    public class CharacterEditor : MonoBehaviour
    {
        ...
        {
            Refresh();
            InitializeDropdown(HeadDropdown, SpriteCollection.Head, Dummy.Head, texture => Dummy.Head = texture, false);
            InitializeDropdown(EarsDropdown, SpriteCollection.Ears, Dummy.Ears, texture => Dummy.Ears = texture, false);
            ...
            InitializeDropdown(EyesDropdown, SpriteCollection.Eyes, Dummy.Eyes, texture => Dummy.Eyes = texture, false);
            InitializeDropdown(MouthDropdown, SpriteCollection.Mouth, Dummy.Mouth, texture => Dummy.Mouth = texture, false);
            InitializeDropdown(BeardDropdown, SpriteCollection.Beard, Dummy.Beard, texture => Dummy.Beard = texture);
            InitializeDropdown(BodyDropdown, SpriteCollection.Body, Dummy.Body, texture => Dummy.Body = texture, false);
		...
		private void InitializeDropdown(Dropdown dropdown, List<Texture2D> sprites, Texture2D texture, Action<Texture2D> callback, bool empty = true)
        {
            dropdown.options = sprites.Select(i => new Dropdown.OptionData(GetSpriteName(i))).ToList();
			if (empty) {
				dropdown.options.Add (new Dropdown.OptionData ("Ekkert valið"));
			}
