...
namespace DoozyUI
{
    ...
    public class UIAnimatorUtil
    {
        ...
        public static string RELATIVE_PATH_ANIMATIONS { get { return QuickEngine.IO.File.GetRelativeDirectoryPath("DoozyUI") + "/Resources/DUI/Animations/"; } }
        ...
        [SerializeField] public static Dictionary<string, List<AnimData>> InAnimDataPresetsDatabase;
        [SerializeField] public static Dictionary<string, List<AnimData>> OutAnimDataPresetsDatabase;
        [SerializeField] public static Dictionary<string, List<LoopData>> LoopDataPresetsDatabase;
        [SerializeField] public static Dictionary<string, List<PunchData>> PunchDataPresetsDatabase;
        ...
        {
            return GetResource<AnimData>(RESOURCES_PATH_IN_ANIM_DATA + presetCategory + "/", presetName).data.Copy();
        }
        public static Anim GetOutAnim(string presetCategory, string presetName)
        {
            return GetResource<AnimData>(RESOURCES_PATH_OUT_ANIM_DATA + presetCategory + "/", presetName).data.Copy();
        }
        public static Loop GetLoop(string presetCategory, string presetName)
        {
            return GetResource<LoopData>(RESOURCES_PATH_LOOP_DATA + presetCategory + "/", presetName).data.Copy();
        }
        public static Punch GetPunch(string presetCategory, string presetName)
        {
            return GetResource<PunchData>(RESOURCES_PATH_PUNCH_DATA + presetCategory + "/", presetName).data.Copy();
        }
        ...
        public static void RefreshInAnimDataPresetsDatabase()
        {
            ...
            InAnimDataPresetsDatabase = new Dictionary<string, List<AnimData>>();
            ...
            {
                EditorUtility.DisplayProgressBar("Refreshing In Animations Database", directories[directoryIndex], ((directoryIndex + 1) / (directories.Count + 2)));
                string[] fileNames = GetInAnimPresetsNamesForCategory(directories[directoryIndex]);
                ...
                for (int fileIndex = 0; fileIndex < fileNames.Length; fileIndex++)
                {
                    ...
                    EditorUtility.DisplayProgressBar("Refreshing In Animations Database", directories[directoryIndex] + " / " + asset.presetName, ((directoryIndex + 1) / (directories.Count + 2)));
        ...
        public static void RefreshOutAnimDataPresetsDatabase()
        {
            ...
            OutAnimDataPresetsDatabase = new Dictionary<string, List<AnimData>>();
            List<string> directories = new List<string>();
            directories = RemoveEmptyPresetFolders(RELATIVE_PATH_OUT_ANIM_DATA, GetOutAnimPresetsDirectories);
            for (int directoryIndex = 0; directoryIndex < directories.Count; directoryIndex++)
            {
                EditorUtility.DisplayProgressBar("Refreshing Out Animations Database", directories[directoryIndex], ((directoryIndex + 1) / (directories.Count + 2)));
                string[] fileNames = GetOutAnimPresetsNamesForCategory(directories[directoryIndex]);
                if (fileNames.Length == 0) { continue; } //empty folder
                ...
                for (int fileIndex = 0; fileIndex < fileNames.Length; fileIndex++)
                {
                    ...
                    if (asset == null) { continue; }
                    EditorUtility.DisplayProgressBar("Refreshing Out Animations Database", directories[directoryIndex] + " / " + asset.presetName, ((directoryIndex + 1) / (directories.Count + 2)));
        ...
        public static void RefreshLoopDataPresetsDatabase()
        {
            ...
            LoopDataPresetsDatabase = new Dictionary<string, List<LoopData>>();
            List<string> directories = new List<string>();
            directories = RemoveEmptyPresetFolders(RELATIVE_PATH_LOOP_DATA, GetLoopPresetsDirectories);
            for (int directoryIndex = 0; directoryIndex < directories.Count; directoryIndex++)
            {
                EditorUtility.DisplayProgressBar("Refreshing Loop Animations Database", directories[directoryIndex], ((directoryIndex + 1) / (directories.Count + 2)));
                ...
                for (int fileIndex = 0; fileIndex < fileNames.Length; fileIndex++)
                {
                    ...
                    if (asset == null) { continue; }
                    EditorUtility.DisplayProgressBar("Refreshing Loop Animations Database", directories[directoryIndex] + " / " + asset.presetName, ((directoryIndex + 1) / (directories.Count + 2)));
        ...
        {
            m_InAnimPresetCategories = new List<string>();
        ...
        {
            m_OutAnimPresetCategories = new List<string>();
        ...
        {
            m_LoopPresetCategories = new List<string>();
        ...
        {
            m_PunchPresetCategories = new List<string>();
        ...
        public static void CreateInAnimPreset(string presetCategory, string presetName, Anim anim)
        {
            CreateAnimDataAsset(RELATIVE_PATH_IN_ANIM_DATA, presetCategory, presetName, anim);
        }
        public static void CreateOutAnimPreset(string presetCategory, string presetName, Anim anim)
        {
            CreateAnimDataAsset(RELATIVE_PATH_OUT_ANIM_DATA, presetCategory, presetName, anim);
        }
        public static void CreateLoopPreset(string presetCategory, string presetName, Loop loop)
        {
            CreateLoopDataAsset(RELATIVE_PATH_LOOP_DATA, presetCategory, presetName, loop);
        }
        public static void CreatePunchPreset(string presetCategory, string presetName, Punch punch)
        {
            CreatePunchDataAsset(RELATIVE_PATH_PUNCH_DATA, presetCategory, presetName, punch);
        }
        ...
        private static void ValidateInAnimPresets()
        {
            ...
            List<string> directories = new List<string>();
            directories = RemoveEmptyPresetFolders(RELATIVE_PATH_IN_ANIM_DATA, GetInAnimPresetsDirectories);
            if (!directories.Contains(DEFAULT_PRESET_CATEGORY) || !InAnimPresetExists(DEFAULT_PRESET_CATEGORY, DEFAULT_PRESET_NAME)) //the default preset category does not exist in the database or the default preset name does not exist in the database-> create it
            {
                refreshDatabase = true;
                CreateInAnimPreset(DEFAULT_PRESET_CATEGORY, DEFAULT_PRESET_NAME, new Anim(Anim.AnimationType.In));
            }
            for (int directoryIndex = 0; directoryIndex < directories.Count; directoryIndex++)
            {
                string[] fileNames = GetInAnimPresetsNamesForCategory(directories[directoryIndex]);
        ...
        private static void ValidateOutAnimPresets()
        {
            ...
            List<string> directories = new List<string>();
            directories = RemoveEmptyPresetFolders(RELATIVE_PATH_OUT_ANIM_DATA, GetOutAnimPresetsDirectories);
            if (!directories.Contains(DEFAULT_PRESET_CATEGORY) || !OutAnimPresetExists(DEFAULT_PRESET_CATEGORY, DEFAULT_PRESET_NAME)) //the default preset category does not exist in the database or the default preset name does not exist in the database-> create it
            {
                refresh = true;
                CreateOutAnimPreset(DEFAULT_PRESET_CATEGORY, DEFAULT_PRESET_NAME, new Anim(Anim.AnimationType.Out));
            }
            for (int directoryIndex = 0; directoryIndex < directories.Count; directoryIndex++)
            {
                string[] fileNames = GetOutAnimPresetsNamesForCategory(directories[directoryIndex]);
        ...
        private static void ValidatePunchPresets()
        {
            ...
            List<string> directories = new List<string>();
            directories = RemoveEmptyPresetFolders(RELATIVE_PATH_PUNCH_DATA, GetPunchPresetsDirectories);
            if (!directories.Contains(DEFAULT_PRESET_CATEGORY) || !PunchPresetExists(DEFAULT_PRESET_CATEGORY, DEFAULT_PRESET_NAME)) //the default preset category does not exist in the database or the default preset name does not exist in the database-> create it
            {
                refresh = true;
                CreatePunchPreset(DEFAULT_PRESET_CATEGORY, DEFAULT_PRESET_NAME, new Punch());
            }
            for (int directoryIndex = 0; directoryIndex < directories.Count; directoryIndex++)
            {
                string[] fileNames = GetPunchPresetsNamesForCategory(directories[directoryIndex]);
