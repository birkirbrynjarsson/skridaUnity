...
namespace DoozyUI
{
    ...
    public class UIManager : QuickEngine.Common.Singleton<UIManager>
    {
        ...
        private static void OnGameEvent(string gameEvent)
        {
            ...
            TriggerTheTriggers(gameEvent, DUI.EventType.GameEvent);
        ...
        /// </summary>
        public static void TriggerTheTriggers(string triggerValue, DUI.EventType triggerType)
        {
            triggerList = GetUiTriggers(triggerValue, triggerType);
            if (triggerList == null || triggerList.Count == 0) { return; }
            for (int i = 0; i < triggerList.Count; i++) { triggerList[i].TriggerTheTrigger(triggerValue); }
        }
        #endregion
